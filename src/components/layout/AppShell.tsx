...
function Logo({ collapsed = false }: { collapsed?: boolean }) {
  return (
    <Link to="/dashboard" className={cn('flex items-center gap-2.5', collapsed && 'justify-center')}>
  ...
  return (
    <>
      <header className="sticky top-0 z-30 flex h-16 items-center justify-between border-b border-white/20 bg-white/80 px-4 backdrop-blur-xl dark:border-white/5 dark:bg-ink-900/80 lg:px-8">
        ...
        <button onClick={() => setSearchOpen(true)} className="hidden w-full max-w-sm items-center gap-3 rounded-xl border border-ink-200 bg-ink-50 px-4 py-2.5 text-left text-sm text-ink-400 transition-colors hover:border-brand-300 hover:text-ink-600 md:flex dark:border-ink-700 dark:bg-ink-800 dark:hover:border-brand-700">
        ...
        <div className="flex items-center gap-1.5 sm:gap-3">
          <button className="relative rounded-lg p-2 text-ink-500 transition-colors hover:bg-ink-100 hover:text-ink-800 dark:hover:bg-ink-800 dark:hover:text-ink-200" aria-label="Notifications">
            <Bell className="h-5 w-5" />
            <span className="absolute right-1.5 top-1.5 h-2 w-2 rounded-full border-2 border-white bg-danger dark:border-ink-900" />
          </button>
          <button onClick={() => setDark((value) => !value)} className="rounded-lg p-2 text-ink-500 transition-colors hover:bg-ink-100 hover:text-ink-800 dark:hover:bg-ink-800 dark:hover:text-ink-200" aria-label="Toggle theme">
          ...
          <DropdownMenu>
            ...
            <DropdownMenuContent align="end" className="w-52">
              ...
              <DropdownMenuSeparator />
              <DropdownMenuItem onClick={() => navigate('/profile')}><CircleUserRound className="mr-2 h-4 w-4" /> Profile</DropdownMenuItem>
      ...
      <Dialog open={searchOpen} onOpenChange={setSearchOpen}>
        <DialogContent><DialogHeader><DialogTitle>Search MathMaster</DialogTitle></DialogHeader><div className="flex items-center gap-3 rounded-xl border border-ink-200 px-4 py-3 dark:border-ink-700"><Search className="h-5 w-5 text-ink-400" /><input autoFocus className="w-full bg-transparent text-sm outline-none" placeholder="Search topics, lessons, quizzes..." /></div><p className="text-xs text-ink-400">Search will be available across your lessons and practice history.</p></DialogContent>
      </Dialog>
    </>
  );
}
...
export function AppShell({ children }: { children: React.ReactNode }) {
  ...
  return <div className="min-h-screen bg-ink-50 dark:bg-ink-900"><div className="flex"><Sidebar collapsed={collapsed} onToggle={() => setCollapsed((value) => !value)} mobileOpen={mobileOpen} onMobileClose={() => setMobileOpen(false)} /><div className="min-w-0 flex-1"><Navbar onMenuClick={() => setMobileOpen(true)} /><main className="mx-auto max-w-7xl p-4 sm:p-6 lg:p-8"><AnimatePresence mode="wait"><motion.div key={window.location.pathname} initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.2 }}>{children}</motion.div></AnimatePresence></main></div></div></div>;
}
