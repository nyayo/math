...
import { ChevronRight, GraduationCap, BarChart3, Bell, Globe, Moon, Download, HelpCircle, MessageSquare, AlertTriangle, FileText, Shield, Palette, LogOut, UserCog, LockKeyhole, Mail } from 'lucide-react';
...
export default function TeacherProfile() {
  ...
  const initials = user?.first_name?.[0]?.toUpperCase() ?? 'T';

  const handleSignOut = async () => { await logout(); navigate('/login'); };

  const groups: MenuGroup[] = [
    { title: 'Teaching', items: [
      { icon: GraduationCap, label: 'My subjects' },
      { icon: GraduationCap, label: 'School info' },
      { icon: GraduationCap, label: 'Class rosters' },
    ]},
    { title: 'Insights', items: [
      { icon: BarChart3, label: 'Teaching analytics', sublabel: 'Coming soon' },
    ]},
    { title: 'Account', items: [
      ...
      <div className="mx-auto max-w-2xl">
        <Card className="rounded-[2rem] p-6 sm:p-8">
          <div className="flex flex-col items-center gap-5 sm:flex-row sm:items-start">
            <div className="relative">
              ...
              <button onClick={() => setEditOpen(true)} className="absolute -bottom-1 -right-1 flex h-8 w-8 items-center justify-center rounded-full bg-accent-500 text-white shadow-soft transition-transform hover:scale-110"><UserCog className="h-4 w-4" /></button>
            </div>
            <div className="flex-1 text-center sm:text-left">
              <h1 className="text-xl font-bold text-ink-900 dark:text-white">{user?.first_name ?? 'Teacher'}</h1>
        ...
        <Dialog open={editOpen} onOpenChange={setEditOpen}>
          <DialogContent>
            ...
            <div className="space-y-4 py-2">
              <div><Label htmlFor="t-name">First name</Label><Input id="t-name" value={firstName} onChange={(e) => setFirstName(e.target.value)} className="mt-2" /></div>
            </div>
            <DialogFooter>
        ...
        <Dialog open={signOutOpen} onOpenChange={setSignOutOpen}>
          <DialogContent className="max-w-sm">
            ...
            <p className="text-sm text-ink-500">You'll need to sign in again to continue teaching.</p>
            <DialogFooter>
              <DialogClose asChild><Button variant="ghost">Cancel</Button></DialogClose>
